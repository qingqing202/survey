...
<div id="edit">
  ...
  <table border="0" align="center" cellpadding="0" cellspacing="0" class="red_xx table_all">
    <tr>
     <td width="93%"><span class="title">用户管理 ></span>  管理员</td>
      ...
      <a href="manageUser.do?action=edit&chnId=${chnId}" class="blue">添加</a> | 
      </c:if>
      <a onclick="return getConfirm('删除后将不能恢复！真的要删除吗？')" href="javascript:document.form.action='manageUser.do?act=delAdmUser';document.form.submit();" class="red">批量删除</a> | 
      <a href="javascript:document.form.action='manageUser.do?act=statusAdmUserAll&stat=0';document.form.submit()" class="red">批量锁定</a> | 
      <a href="javascript:document.form.action='manageUser.do?act=statusAdmUserAll&stat=1';document.form.submit()" class="red">批量激活</a></td>
    </tr>
  </table>
  <table border="0" align="center" cellpadding="0" cellspacing="0" class="table_all">
    <tr>
  ...
  <table width="90%" align="center" cellspacing="1" class="table table_all" id="table1">
    ...
    <tr onmouseover="over()" onclick="change()" onmouseout="out()" class="td_bg">
      ...
      <td align="center" id="td_${user.id}"><a href='javascript:void(0);' onclick="chgStatus_ajax('manageUser.do?act=statusAdmUser&id=${user.id}&stat=${users.ipass}','td_${user.id}')">
	 ...
	 <s:else>
      ...
      <c:if test="${fn:indexOf(permit,'view')>0}">
        <a href='manageUser.do?action=edit&userId=${user.id}&chnId=${chnId}'>改</a>  
      </c:if>
      <c:if test="${fn:indexOf(permit,'delete')>0}">  
        <a onclick="javascript:return getConfirm('删除后将不能恢复！真的要删除吗');" href='manageUser.do?action=delAdmin&userId=${user.id}&chnId=${chnId}'>删</a> 
      </c:if>  
       <c:if test="${fn:indexOf(permit,'add')>0}">  
        <a href='manageUser.do?action=chagPwd&userId=${user.id}&chnId=${chnId}'>密改</a> 
      </c:if>  
       </td>
      </tr>
      </s:iterator>
