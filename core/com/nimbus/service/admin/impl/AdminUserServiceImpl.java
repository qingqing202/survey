package com.nimbus.service.admin.impl;
...
import com.nimbus.entity.admin.AdminRole;
...
public class AdminUserServiceImpl implements AdminUserService {
  ...
  public void setSessionFactory(
    ...
    this.roleDao = new PagerHibernateTemplate<AdminRole, Long>(sessionfactory,
        AdminRole.class);
    ...
    this.userDao = new SimpleHibernateTemplate<AdminUser, Long>(sessionfactory,
        AdminUser.class);
  ...
  public List<AdminRoleFunction> getFunctionList(Long roleId) {
    ...
    List<AdminRoleFunction> list = (List<AdminRoleFunction>) functionDao.find(
        "from AdminRoleFunction where role.id=?", new Object[] { roleId });
    if (list.isEmpty()) {
      return null;
    }
    return list;
  ...
  public void delPermissions(Long userId) {
    Query query = permissionsDao
        .createQuery("delete from AdminPermissions where adminUser.id=:id");
    query.setParameter("id", userId);
    query.executeUpdate();
  }

  public List<AdminPermissions> getPermissionsList(Long roleId) {
    List<AdminPermissions> list = (List<AdminPermissions>) permissionsDao.find(
        "from AdminPermissions where adminRole.id=?", new Object[] { roleId });
    if (list.isEmpty()) {
      return null;
    }
    return list;
  ...
  public String getPermissions(Long roleId, Long channelId) {
    List<AdminPermissions> list = (List<AdminPermissions>) permissionsDao.find(
        "from AdminPermissions where adminRole.id=? and channel.id=?",
  ...
  public List<AdminRoleFunction> getRoleFunctionList(Long roleId, Long parentId) {
    List<AdminRoleFunction> list = (List<AdminRoleFunction>) functionDao
        .find(
            "from AdminRoleFunction where role.id=? and channel.parentId=? order by channel.id desc",
  ...
  public List<AdminRoleFunction> getRoleFunctionList(Long roleId) {
    List<AdminRoleFunction> list = (List<AdminRoleFunction>) functionDao
        .find(
            "from AdminRoleFunction where role.id=? and channel.parentId>0 order by channel.id desc",
            new Object[] { roleId });
    if (list.isEmpty()) {
      return null;
    }
  ...
  public List<AdminPermissions> getPermissionsUrlList(Long userId) {
    List<AdminPermissions> list = (List<AdminPermissions>) permissionsDao.find(
        "from AdminPermissions where adminRole.id=? ", new Object[] { userId });
    if (list.isEmpty()) {
      return null;
    }
    return list;
  ...
  public void delPermissions(String chid, Long roleId) {
    Query query = permissionsDao
        .createQuery("delete from AdminPermissions where adminRole.id=:id and channel.id not in("
            + chid + ")");
    query.setParameter("id", roleId);
    query.executeUpdate();
  }
